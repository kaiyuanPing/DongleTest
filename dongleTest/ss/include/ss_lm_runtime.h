...
*   @param[out] auth          认证 login 函数返回是否正确，含有 ECC签名 和返回值加密，不使用可以填 NULL。
*   @return     成功返回 SS_OK，失败返回相应的错误码
*   @remarks
*   - #slm_login 函数详细说明：
*       -# 会自动查找跨锁查找许可句柄。
...
*               -# 敏感数据内存不泄密、无法篡改。
*               -# 可以跨线程安全交互数据。
*               -# APP软件、许可、Virbox许可服务 三者强耦合，软件防止被破解能力极高。（黑客需要手工剥离和重建才能使软件）
*
*   @code
*       {
*           SS_UINT32 status = SS_OK;
...
/*!
*   @brief      将D2C包进行升级，需指定加密锁唯一序列号，d2c包 由 @link D2CAPI @endlink 生成
