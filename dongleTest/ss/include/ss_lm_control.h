...
*   @param[in]  bind_c2d        解绑 C2D 信息
*   @param[in]  c2d_length      C2D 长度
*   @return     成功返回 SS_OK ，失败返回相应错误码
*   @remarks    此函数通过错误码判断许可解绑是否成功，SS_OK 表示解绑成功，其他表示解绑失败。如果解绑失败，可在深思开发者中心进行手动解绑
*/
...
/*!
*   @brief      将D2C包进行升级，D2C包 由 @link D2CAPI @endlink 生成
*   @param[in]  d2c_pkg     D2C文件数据
*   @param[out] error_msg   错误信息，需要调用 #slm_ctrl_free 释放
*   @return     成功返回 SS_OK ，失败返回错误码
*   @remarks:   D2C包 内可能包含多条数据，因此返回的错误码信息也是 json 数组结构，需要分别解析
*   @code
*     [
...
/*!
*   @brief      将 D2C包 进行升级，需指定加密锁唯一序列号，D2C包 由 @link D2CAPI @endlink 生成
*   @param[in]  lock_sn     锁号（唯一序列号,十六进制字符串）
*   @param[in]  d2c_pkg     D2C 文件数据
*   @param[out] error_msg   错误信息，不使用需要调用 #slm_ctrl_free 释放
*   @return     成功返回 SS_OK ，失败返回错误码
*   @remarks:  D2C 包内可能包含多条数据，因此返回的错误码信息也是 json 数组结构，需要分别解析
*   @code
*     [
*       {"pkg_order":1, "pkg_desc":"package decription.", "status": 0},
*       {"pkg_order":2, "pkg_desc":"package decription.", "status": 0}
...
*               例如：{"urls":{"cloud_runtime_url":"http://rt.senseyun.com", "slock_url":"http://sl.senseyun.com"}}
*   @return     成功返回SS_OK，失败返回相应错误码
